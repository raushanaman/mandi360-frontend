...
const Login = () => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (data.role === 'merchant') navigate('/dashboard');
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-slate-900 flex-col justify-between overflow-hidden p-12">
        ...
        <div className="relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
            ...
            <div className="space-y-4">
              {PERKS.map((p, i) => (
                <motion.div key={p.text} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + i * 0.1 }} className="flex items-center gap-3">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="relative z-10 bg-white/10 backdrop-blur rounded-2xl p-5 border border-white/10">
          <p className="text-white/80 text-sm italic mb-3">"Mandi-360 changed how I shop. Everything from my local market is now just a tap away!"</p>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 bg-slate-50">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="w-full max-w-sm">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
            ...
            <p className="text-slate-500 text-sm mb-6">Sign in to your Mandi-360 account.</p>
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" name="email" value={form.email} onChange={handleChange} placeholder="you@example.com"
                    className="w-full bg-slate-50 border border-slate-200 pl-10 pr-4 py-3 rounded-xl text-sm outline-none focus:border-slate-400 focus:bg-white transition-all" required />
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  <Link to="/forgot-password" className="text-xs font-bold text-red-500 hover:text-red-600 transition-colors">Forgot password?</Link>
                </div>
                <div className="relative">
                  ...
                  <input type={showPassword ? 'text' : 'password'} name="password" value={form.password} onChange={handleChange} placeholder="Enter your password"
                    className="w-full bg-slate-50 border border-slate-200 pl-10 pr-11 py-3 rounded-xl text-sm outline-none focus:border-slate-400 focus:bg-white transition-all" required />
